...
load_dotenv()
import wandb
from huggingface_hub.hf_api import HfFolder  # pyright: ignore

# NOTE: Login to Wandb and HuggingFace
wandb.login(key=str(os.getenv("WANDB_KEY")))
HfFolder.save_token(token=str(os.getenv("HF_TOKEN")))
...
def main(cfg: DictConfig) -> None:
    ...
    # pprint(cfg.experiment.callbacks.checkpoint.dirpath)
    model_paths = []
    best_scores = []
    ...
    for fold in range(cfg.experiment.datamodule.num_folds):
        ...
        model_paths.append(trainer.checkpoint_callback.best_model_path)
        best_scores.append(trainer.checkpoint_callback.best_model_score.item())

    wandb.finish()
    # checkpoint_path = model_paths[best_scores.index(max(best_scores))]
    model_dir = str(cfg.experiment.callbacks.checkpoint.dirpath)

    save_and_push_best_model(model_dir=model_dir)
